...
{
    /// <summary>
    /// Copy of WindowBase2 that:
    ///  - Properly handles resizing
    /// </summary>
    public abstract class InternalWindowBase2 : Container, IWindow, IViewContainer
    {
        ...
        private static readonly SettingCollection _windowSettings = ItemSearchModule.Instance.GetSettingCollection();
        ...
        public static void UpdateWindowBaseDynamicHUDCombatState(InternalWindowBase2 wb)
        {
        ...
        public static void UpdateWindowBaseDynamicHUDLoadingState(InternalWindowBase2 wb)
        {
        ...
        {
            // Update all bounds according to Size
            this.WindowRegion = new Rectangle(WindowRegion.Left, WindowRegion.Top, Size.X, Size.Y - STANDARD_TITLEBAR_HEIGHT);
            this.WindowRelativeContentRegion = WindowRegion.WithPadding(Thickness.Zero - WindowContentMargin);
            this.TitleBarBounds = new Rectangle(0, 0, WindowRegion.Width, STANDARD_TITLEBAR_HEIGHT);

            _backgroundDrawBounds = new Rectangle(-WindowRegion.Left, -WindowRegion.Top + STANDARD_TITLEBAR_HEIGHT, Size.X, Size.Y - STANDARD_TITLEBAR_HEIGHT);
            ...
            this.Padding = new Thickness(Math.Max(WindowRegion.Top - STANDARD_TITLEBAR_HEIGHT, STANDARD_TITLEBAR_VERTICAL_OFFSET), // We have to include the padding of the titlebar just in case
                                         WindowBackground.Width - WindowRegion.Right,
                                         WindowBackground.Height - WindowRegion.Bottom + STANDARD_TITLEBAR_HEIGHT,
                                         WindowRegion.Left);
            this.ContentRegion = new Rectangle(WindowRelativeContentRegion.X - (int)this.Padding.Left,
                                               WindowRelativeContentRegion.Y + STANDARD_TITLEBAR_HEIGHT - (int)this.Padding.Top,
                                               WindowRelativeContentRegion.Width,
                                               WindowRelativeContentRegion.Height);
        ...
        protected Thickness WindowContentMargin { get; set; }

        protected void ConstructWindow(Texture2D background, Rectangle windowRegion, Thickness contentMargin)
        {
            this.WindowBackground = background;

            this.WindowRegion = windowRegion;
            this.WindowContentMargin = contentMargin;
            this.WindowRelativeContentRegion = windowRegion.WithPadding(Thickness.Zero - contentMargin);
            this.TitleBarBounds = new Rectangle(0, 0, windowRegion.Width, STANDARD_TITLEBAR_HEIGHT);
        ...
        {
            int sideBarWidth = this.ShowSideBar ? SIDEBAR_WIDTH : 0;
            var destinationRect = new Rectangle(_backgroundDrawBounds.X + sideBarWidth, _backgroundDrawBounds.Y, _backgroundDrawBounds.Width - sideBarWidth, _backgroundDrawBounds.Height);
            var sourceRect = new Rectangle(sideBarWidth, 0, this.WindowBackground.Width - sideBarWidth, this.WindowBackground.Height);
            spriteBatch.DrawOnCtrl(this, this.WindowBackground, destinationRect, sourceRect);
        }
